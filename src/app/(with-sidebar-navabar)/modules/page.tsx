...
function Page() {
  ...
  const handleDelete = async (_id: string) => {
    try {
      const res = await axios.delete(`/api/modules/delete/${_id}`);
      setModules((prev) => prev.filter((module) => module._id !== _id)); // Update state after deletion
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div className="flex-1 p-4">
      ...
      <h1 className="text-3xl p-8 font-bold text-etuwaCustom-db">Modules</h1>
      ...
      <div className="p-4">
        <div className="bg-gradient-to-r  from-etuwaCustom-b to-etuwaCustom-db  text-white mb:flex mb:items-center mb:justify-between sm:grid sm:grid-cols-[repeat(auto-fit,minmax(120px,1fr))] sm:gap-4 p-4 bg-gray-100 rounded-lg shadow-md font-semibold ">
          <h3 className="text-center">Module</h3>
        ...
        {modules.map((module) => (
          <div
            ...
            key={module._id}
          >
            <Link href={`/modules/${module._id}`} className="text-center ">
              {module.module_name}
            </Link>
            ...
            <div className="flex items-center justify-center space-x-2">
              ...
              <Link
                ...
                href={`/modules/${module._id}/update`}
                replace
              >
                <svg
                  className="h-5 w-5 text-gray-100"
