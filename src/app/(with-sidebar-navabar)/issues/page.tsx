...
function Page() {
  ...
  const handleDelete = async (_id: string) => {
    try {
      const res = await axios.delete(`/api/issues/delete/${_id}`);
      setIssues((prev) => prev.filter((issue) => issue._id !== _id)); // Update state after deletion
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div className="flex-1 p-4">
      ...
      <h1 className="text-3xl p-8 font-bold text-etuwaCustom-db">Issues</h1>
      <div className="p-4">
        <div className="bg-gradient-to-r  from-etuwaCustom-b to-etuwaCustom-db  text-white mb:flex mb:items-center mb:justify-between sm:grid sm:grid-cols-[repeat(auto-fit,minmax(120px,1fr))] sm:gap-4 p-4 bg-gray-100 rounded-lg shadow-md font-semibold ">
          <h3 className="text-center">Issue</h3>
        ...
        {issues.map((issue) => (
          <div
            ...
            key={issue._id}
          >
            <Link href={`/issues/${issue._id}`} className="text-center ">
              {issue.issue}
            </Link>
