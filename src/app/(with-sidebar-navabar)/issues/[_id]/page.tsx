...
async function fetchIssue(_id: string) {
  const res = await fetch(`http://localhost:3000/api/issues/${_id}`, {
...
export default async function IssueDetailsPage({
  ...
  const data = await fetchIssue(params._id);
      ...
      <div className="flex p-4 justify-between bg-white rounded-lg shadow-md m-1 bg-gradient-to-l from-white to-etuwaCustom-lb">
        ...
        <div>
          ...
          <DeleteButton
            ...
            _id={params._id}
            api={`/api/issues/delete/${params._id}`}
            redirectLink="/issues"
          >
            Delete
          </DeleteButton>
      ...
      <div className="flex flex-col p-10 shadow-md m-1 rounded-lg bg-gradient-to-l from-white to-etuwaCustom-lb">
        ...
        <div className="flex flex-col  ">
          <div className="flex p-5 items-center text-etuwaCustom-db font-normal ">
            <div className="w-40">Issue:</div>
            <span className="ml-2 font-semibold">{issue.issue}</span>
          </div>
          <div className="flex p-5 items-center text-etuwaCustom-db font-normal ">
            <div className="w-40">Status:</div>
            <span className="ml-2 font-semibold">{issue.status}</span>
          </div>
        </div>
      </div>
    </div>
